...
export const MediaReviewModal: React.FC<MediaReviewModalProps> = ({
  ...
  useEffect(() => {
    // Browsers automatically handle EXIF orientation for display
    // Manual rotation here is only for user adjustments
    setRotation(0);
  }, [file, type]);
        ...
        <div className="flex flex-col items-center mb-4">
          <div className="bg-white/10 rounded-full p-1 flex relative">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all ${privacy === 'public' ? 'bg-white text-black' : 'text-white/60 hover:text-white'
                }`}
            >
              <Globe size={12} />
              {t('public')}
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all ${privacy === 'private'
                ? 'bg-white text-black'
