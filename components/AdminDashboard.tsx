...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const renderFeedback = () => {
    // Get all beta user access data
    const betaUserAccess = BetaTestingManager.getAllBetaUserAccess();
    const allUsersWithFeedback = Object.values(betaUserAccess).filter(user => user.feedbackSubmitted);
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">Beta Feedback Management</h3>
        ...
        <div className="p-6 border-b border-slate-100">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
              <div className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-1">Total Beta Users</div>
              <div className="text-2xl font-black text-slate-900">{stats.totalBetaUsers}</div>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
              <div className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-1">Feedback Submitted</div>
              <div className="text-2xl font-black text-slate-900">{stats.feedbackSubmitted}</div>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="bg-slate-100 text-slate-600 text-xs font-bold px-2.5 py-1 rounded-full">{allUsersWithFeedback.length} submissions</span>
          </div>

          {allUsersWithFeedback.length === 0 ? (
            ...
            <div className="space-y-4">
              {allUsersWithFeedback.map((userAccess, index) => {
                // Find the corresponding user
                const user = users.find(u => u.id === userAccess.userId);

                return (
                  <div key={userAccess.userId} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h5 className="font-bold text-slate-900">{user ? user.name : `User ${userAccess.userId}`}</h5>
                          <p className="text-sm text-slate-500">{user ? user.email : 'Anonymous user'}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded-full border border-green-200">FEEDBACK SUBMITTED</span>
                        <span className="text-xs text-slate-400">{new Date(userAccess.grantedAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-lg border border-slate-100">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-bold text-slate-900">Beta Program Participation</span>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                        <div className="bg-slate-50 p-2 rounded">
                          <span className="text-slate-500">Version:</span>
                          <span className="font-bold text-slate-900 ml-1">{userAccess.version}</span>
                        </div>
                        <div className="bg-slate-50 p-2 rounded">
                          <span className="text-slate-500">Features:</span>
                          <span className="font-bold text-slate-900 ml-1">{userAccess.features.length}</span>
                        </div>
                        <div className="bg-slate-50 p-2 rounded">
                          <span className="text-slate-500">Joined:</span>
                          <span className="font-bold text-slate-900 ml-1">{new Date(userAccess.grantedAt).toLocaleDateString()}</span>
                        </div>
                        <div className="bg-slate-50 p-2 rounded">
                          <span className="text-slate-500">Role:</span>
                          <span className="font-bold text-slate-900 ml-1">{user ? user.role : 'Unknown'}</span>
                        </div>
                      </div>
                    </div>

                    <div className="mt-3 flex items-center gap-2">
                      <button className="flex-1 py-2 bg-slate-100 text-slate-700 font-bold rounded-lg hover:bg-slate-200 transition-colors flex items-center justify-center gap-2">
                        <MessageSquare size={16} />
                        View Full Feedback
                      </button>
                      <button className="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Positive Feedback">
                        <ThumbsUp size={18} />
                      </button>
                      <button className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Negative Feedback">
                        <ThumbsDown size={18} />
                      </button>
                    </div>
                  </div>
